...
# 用户登录
@user_router.post("/login", summary='用户登录', tags=['user:用户'],
...
# 用户注销
@user_router.post('/logout', summary='用户注销', tags=['user:用户'], status_code=204)
...
# 创建用户
@user_router.post('/users', summary='创建用户', tags=['user:用户'], status_code=status.HTTP_201_CREATED,
...
# 查看所有用户(分页)
@user_router.get('/users', summary='查看所有用户（分页）', tags=['user:用户'],
                 responses={200: {'model': GetUsersResSchema}})
...
# 修改用户信息
@user_router.patch('/users/{user_id}', summary='更改用户信息', tags=['user:用户'],
...
# 删除用户
@user_router.delete('/users/{user_id}', summary='删除单个用户', tags=['user:用户'],
                    status_code=status.HTTP_204_NO_CONTENT)
...
# 删除多个用户
@user_router.delete('/users', summary='删除多个用户', tags=['user:用户'], status_code=status.HTTP_204_NO_CONTENT)
...
# 创建角色
@user_router.post('/roles', summary='创建角色', tags=['user:角色'], status_code=status.HTTP_201_CREATED)
...
# 查看角色（分页）
@user_router.get('/roles', summary='查看角色（分页）', tags=['user:角色'], responses={200: {'model': GetRolesResSchema}})
...
# 查看角色的所有权限
@user_router.get('/roles/{role_id}/permissions', tags=['user:角色'], summary='查看角色的所有权限',
...
# 更改角色信息
@user_router.patch('/roles/{role_id}', summary='更改角色信息', tags=['user:角色'],
                   status_code=status.HTTP_204_NO_CONTENT)
...
# 更改角色拥有的权限
@user_router.put('/roles/{role_id}/permissions', summary='更改角色拥有的权限', tags=['user:角色'],
                 status_code=status.HTTP_204_NO_CONTENT)
async def update_role_permissions(role_id: int, data: list[int] = Body(),
                                  current_user=Depends(get_current_user({'roles': 'update'}))):
    try:
...
# 查看所有权限
@user_router.get('/permissions', summary='查看所有权限', tags=['user:角色'],
